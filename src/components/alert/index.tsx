...
import { defaultAlertClasses } from './default'
enum AlertEnum {
  success,
  danger,
  warning,
...
export const Alert = React.forwardRef<HTMLDivElement, AlertProps>(function Alert(props, ref) {
  ...
  const {
    theme: { alert = defaultAlertClasses },
    ...
    <div className={cls} role="alert" ref={ref} {...other}>
      {onClose && (
        <button onClick={onClose} className="absolute top-0 right-0 mt-4 mr-4" aria-label="close">
          <svg className={closeCls} fill="currentColor" viewBox="0 0 20 20" role="img" aria-hidden="true">
            <path
